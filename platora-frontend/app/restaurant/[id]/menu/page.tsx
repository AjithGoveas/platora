...
export default function MenuPage() {
        ...
        async function loadItems() {
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    setError(`Failed to load menu: ${res.status} ${text}`);
                    return;
                }
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            if (!res.ok) {
                const msg = (body && (body.error || body.message)) || (editingId ? 'Failed to update item' : 'Failed to add item');
                setError(String(msg));
                return;
            }
    ...
    async function handleDelete(itemId: number | string) {
        ...
        try {
            ...
            const body = await res.json().catch(() => ({}));
            if (!res.ok) {
                const msg = (body && (body.error || body.message)) || 'Failed to delete item';
                setError(String(msg));
                return;
            }
    ...
    // UI: if `id` is missing, show an inline error but keep hooks consistent
    if (!id) {
        return (
