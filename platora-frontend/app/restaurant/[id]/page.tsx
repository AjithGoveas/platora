...
export default async function RestaurantPage({params}: Props) {
    ...
    try {
        ...
        return (
            <main className="mt-24 px-4 md:px-8">
                <div className="text-center text-sm text-rose-600">Failed to load restaurant. Please try again later.
                </div>
                <div className="mt-4 text-center">
                    <Link href="/" className="text-rose-500 underline">Back to home</Link>
                </div>
                ...
                <div className="flex items-center gap-4 md:col-span-2">
                    ...
                    <div>
                        <div className="flex items-center gap-3">
                            ...
                            <span
                                className="text-sm text-muted-foreground">{restaurant.is_active ? 'Open' : 'Closed'}</span>
                        </div>

                        <div className="mt-2 text-sm text-muted-foreground flex flex-wrap items-center gap-3">
                            <span className="inline-flex items-center gap-1"><MapPin className="h-4 w-4"
                                                                                     aria-hidden/> <span>{restaurant.address || 'Address not provided'}</span></span>
                            {restaurant.phone &&
                                <span className="inline-flex items-center gap-1"><Phone className="h-4 w-4"
                                                                                        aria-hidden/> <a
                                    href={`tel:${restaurant.phone}`} className="hover:underline">{restaurant.phone}</a></span>}
                        </div>

                        <div
                            className="mt-2 text-sm text-muted-foreground">Owner: {restaurant.owner_name || restaurant.owner_email || '—'}</div>
                        <div className="mt-3 flex gap-2 items-center">
                            <span
                                className={"px-2 py-1 rounded-full text-xs font-medium " + (restaurant.is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600')}>{restaurant.is_active ? 'Open' : 'Closed'}</span>
                            <span
                                className="px-2 py-1 rounded-full text-xs bg-amber-100 text-amber-800 inline-flex items-center gap-1"><Star
                                className="h-4 w-4" aria-hidden/> <span className="sr-only">Rating</span>4.5</span>
                        </div>
                    </div>
                </div>

                <div className="flex gap-2 md:justify-end">
                    <Link href="/" aria-label="Back to restaurants"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <section className="lg:col-span-2">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <div
                            className="text-sm text-muted-foreground">{menu.length} item{menu.length !== 1 ? 's' : ''}</div>
                    </div>
                    ...
                    {available.length > 0 && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {available.map((item) => (
                                <MenuItemCard key={String(item.id)} item={item} restaurantId={restaurant.id}/>
                            ))}
                        </div>
                    )}
                            ...
                            <div className="mt-3 space-y-2">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="font-semibold">
                                        <time
                                            dateTime={restaurant.created_at ?? undefined}>{formatDateSafe(restaurant.created_at)}</time>
                                    </div>
                                </div>
                            </div>
